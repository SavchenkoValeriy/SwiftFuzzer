...
public struct SwiftFuzzerCore {
    ...
    public static func run(options: SwiftFuzzerOptions) async throws {
        ...
        if !targetExists {
            ...
            let error = DiagnosticError.targetNotFound(
                options.target, availableTargets: availableTargets)
    ...
    static func linkDependenciesToFuzzBuild(
        ...
        UserInterface.showStep(
            "Dependencies: \(allDependencies.sorted().joined(separator: ", "))", isSubStep: true)
        ...
        let regularModulesDir = regularBuildPath.appending(
            components: buildConfig.dirname, "Modules")
        let regularPlatformModulesDir = regularBuildPath.appending(
            components: "arm64-apple-macosx", buildConfig.dirname, "Modules")
        let regularToolModulesDir = regularBuildPath.appending(
            components: "host", buildConfig.dirname, "Modules-tool")

        for dependency in allDependencies {
            // Try all possible module locations
            let modulesLocations = [
                regularModulesDir, regularPlatformModulesDir, regularToolModulesDir,
            ]
            ...
            for modulesDir in modulesLocations {
                ...
                if fileSystem.exists(sourceModule) && !fileSystem.exists(targetModule) {
                    do {
                        try fileSystem.createSymbolicLink(
                            targetModule, pointingAt: sourceModule, relative: false)
                ...
                for ext in [".swiftdoc", ".swiftsourceinfo", ".abi.json"] {
                    ...
                    if fileSystem.exists(sourceFile) && !fileSystem.exists(targetFile) {
                        do {
                            try fileSystem.createSymbolicLink(
                                targetFile, pointingAt: sourceFile, relative: false)
                        } catch {
            ...
            for libDir in libLocations {
                for ext in [".a", ".dylib"] {
                    ...
                    if fileSystem.exists(sourceLib) && !fileSystem.exists(targetLib) {
                        do {
                            try fileSystem.createSymbolicLink(
                                targetLib, pointingAt: sourceLib, relative: false)
    ...
    static func buildTargetWithFuzzInstrumentation(
        ...
        let regularTargetBuildDir = regularBuildPath.appending(
            components: "arm64-apple-macosx", buildConfig.dirname)
        ...
        moduleSearchPaths.append(contentsOf: [
            ...
            "-I",
            fuzzBuildPath.appending(component: buildConfig.dirname).appending(component: "Modules")
                .pathString,
        ])
        ...
        let allLinkedDeps = Set(
            targetModule.dependencies.compactMap { dep in
                switch dep {
                case .module(let module, _):
                    return module.name
                case .product(let product, _):
        ...
        let outputStream = try! ThreadSafeOutputByteStream(
        ...
        if sourceTargetModule.type == .executable {
            let error = DiagnosticError(
                ...
                description:
                    "SwiftFuzzer currently only supports library targets with @fuzzTest functions.",
                possibleCauses: [
        ...
        for dependency in targetDependencies {
            if case .module(let depModule, _) = dependency {
                if depModule.type == .macro {
                    let macroPluginPath = regularHostBuildDir.appending(
                        component: "\(depModule.name)-tool")
                    if fileSystem.exists(macroPluginPath) {
                        macroPluginArgs.append(contentsOf: [
                            "-load-plugin-executable",
                            "\(macroPluginPath.pathString)#\(depModule.name)",
                        ])
        ...
        var compileArgs = [
            ...
            "-emit-module",
            "-emit-module-path",
            modulesDir.appending(component: "\(target).swiftmodule").pathString,
        ...
        compileProcess.launchPath = toolchain.swiftCompilerPath.pathString
        ...
        if compileProcess.terminationStatus != 0 {
            let friendlyError = DiagnosticError.compilationFailed(
                "Swift compiler failed with exit code: \(compileProcess.terminationStatus)")
            try UserInterface.reportError(friendlyError)
        }
        ...
        linkProcess.launchPath = toolchain.swiftCompilerPath.pathString
        ...
        if linkProcess.terminationStatus != 0 {
            let friendlyError = DiagnosticError.compilationFailed(
                "Swift linker failed with exit code: \(linkProcess.terminationStatus)")
            try UserInterface.reportError(friendlyError)
        }
    }
        ...
        Task {
            do {
                for try await line in outputData.bytes.lines {
                    ...
                    if line.contains("CRASHED") || line.contains("AddressSanitizer")
                        || line.contains("ERROR")
                    {
    ...
    static func handleCrashDetected(
        crashPath: Basics.AbsolutePath, packagePath: Basics.AbsolutePath
    ) async {
        UserInterface.showStep("🚨 CRASH DETECTED! Preparing analysis...")
    }

    // Analyze crash artifacts and provide user-friendly reports
    static func analyzeCrashArtifacts(
        crashPath: Basics.AbsolutePath, packagePath: Basics.AbsolutePath
    ) async {
        let fileSystem = Basics.localFileSystem
            ...
            fileSystem.exists(analysisAbsPath)
        {
            do {
                ...
                UserInterface.showStep(
                    "Error reading crash analysis file: \(error)", isSubStep: true)
            }
        }
        ...
        do {
            let crashFiles = try fileSystem.getDirectoryContents(crashPath)
                .filter {
                    $0.hasPrefix("crash-") || $0.hasPrefix("leak-") || $0.hasPrefix("timeout-")
                }
                .sorted()

            if crashFiles.isEmpty {
                UserInterface.showStep(
                    "No crash artifacts found in \(crashPath.pathString)", isSubStep: true)
                UserInterface.reportWarning(
                    DiagnosticWarning(
    ...
    static func analyzeSingleCrash(
        ...
        if let report = FuzzTestRegistry.analyzeCrash(fromData: crashData) {
            ...
            let crash = CrashReport(
                ...
                reproductionCode: extractReproductionCode(from: report)
                    ?? "// Unable to generate reproduction code",
                rawInputHex: crashData.prefix(32).map { String(format: "%02X", $0) }.joined(
                    separator: " "),
        ...
        let reproductionPath = packagePath.appending(component: "crash").appending(
            component: "CrashReproduction\(index).swift")
        if FuzzTestRegistry.createReproductionTest(
            ...
            UserInterface.showSuccess(
                "Auto-generated reproduction test: \(reproductionPath.pathString)")
        }
    }
